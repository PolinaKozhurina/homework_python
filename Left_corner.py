...
for i in range(0, 4):
    ...
    def calc_left_corner(u_arr, u_arr_k):
        h = 1 / num
        tau = 1 / (10 * num)
    ...
    for t in range(int(T*(10*num))):
        for j in range(1, num):
            u_arr_new[j] = calc_left_corner(u_arr[j - 1], u_arr[j])
        u_arr = u_arr_new.copy()
    for t in range(int(T * (num))):
            for j in range(1, num):
                u_arr_new_an[j] = calc_left_corner_analitic(u_arr_an[j - 1], u_arr_an[j])
